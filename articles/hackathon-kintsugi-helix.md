...
**SREチーム・DevOpsエンジニア・オンコール担当者**を対象としています。特に、本番環境のJava/Spring Bootアプリケーションを運用するチームが主なユーザーです。

### 課題

本番環境での障害対応は、多くの現場で以下のような問題を抱えています：

- **深夜のオンコール対応**: エラー検知から原因特定・修正まで、エンジニアが手動で対応している
- **MTTR（平均復旧時間）の長さ**: ログ調査→原因分析→修正→テスト→デプロイに数時間〜数日かかる
- **属人化**: ベテランエンジニアでないと対応できない障害が多い
- **同じバグの再発**: 過去の修正知見が共有されず、同種の問題が繰り返し発生する

### ソリューション

Kintsugi-Helixは、この障害対応プロセスを**完全に自動化**します：

| 従来の手動対応 | Kintsugi-Helix |
|---------------|----------------|
| エンジニアがログを目視確認 | Cloud Loggingから自動検知 |
| 経験に基づく原因推測 | Gemini 2.5 Proが根本原因を分析 |
| 手動でテストコードを書く | JUnitテストを自動生成・実行 |
| 修正コードを人手で作成 | AIが修正を生成し自己検証ループで品質担保 |
| PRレビュー後にマージ | Blast Radius分析で低リスクなら自動マージ |
| 修正知見がSlackに埋もれる | ナレッジベースに自動蓄積、次回から即対応 |

**結果として、数時間かかっていた障害復旧を数分で完了**できます。
